...
import { Star, Users, Heart, Briefcase, Gift, Send } from 'lucide-react';
...
const Events: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900">
      {/* Hero Section */}
      <section className="relative h-[75vh] md:h-[32rem] flex items-center justify-center w-full overflow-hidden">
        <div 
        ...
        <div className="relative z-10 text-center text-white max-w-2xl mx-auto px-4 sm:px-6">
          <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold mb-3 sm:mb-4 drop-shadow-lg">
            Elevate Your Event with a Coffee Bar
          </h1>
          <p className="text-lg sm:text-xl md:text-2xl text-amber-100 drop-shadow-md max-w-xl mx-auto">
      ...
      <div className="relative -mt-16 z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl shadow-xl sm:shadow-2xl overflow-hidden p-6 sm:p-8 lg:p-12">
          <div className="text-center">
            <h2 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white sm:text-4xl">
              How It Works
            </h2>
            <p className="mt-2 sm:mt-3 max-w-2xl mx-auto text-lg sm:text-xl text-gray-500 dark:text-gray-300 sm:mt-4">
          ...
          <div className="mt-8 sm:mt-12 grid grid-cols-1 gap-6 sm:gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div key={index} className="flex flex-row sm:flex-col items-center text-left sm:text-center p-4 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                <div className="flex items-center justify-center h-12 w-12 sm:h-16 sm:w-16 rounded-full bg-primary text-white text-xl sm:text-2xl font-bold border-4 border-white dark:border-gray-800 shadow-lg flex-shrink-0">
                  {index + 1}
                </div>
                <div className="ml-4 sm:ml-0">
                  <h3 className="sm:mt-6 text-base sm:text-lg font-medium text-gray-900 dark:text-white">
                    {step.title}
                  </h3>
                  <p className="mt-1 sm:mt-2 text-sm sm:text-base text-gray-500 dark:text-gray-300">
                    {step.description}
                  </p>
                </div>
              </div>
                    ...
                    <div className={`p-2 rounded-lg transition-colors duration-300 ${
                      ...
                      {eventType.icon &&
                        React.createElement(eventType.icon.type, {
                          className: `h-6 w-6 transition-all duration-300 group-hover:-rotate-[18deg] group-hover:scale-110 ${formData.eventType === eventType.id ? 'text-primary dark:text-primary-400' : 'text-gray-600 dark:text-gray-300'} group-hover:text-amber-300`,
                          'aria-hidden': 'true'
                        })
                      }
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          {/* Inquiry Form - Moved to top for mobile */}
          <div className="order-1 lg:order-2">
            <div id="inquiry" className="bg-white dark:bg-gray-800 shadow-xl rounded-3xl p-6 sm:p-8 lg:p-12">
              <div className="text-center mb-8">
                <h2 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white">Request a Quote</h2>
                <p className="mt-3 text-gray-500 dark:text-gray-300">Fill out the form below and we'll get back to you within 24 hours.</p>
              </div>
              ...
              {submitSuccess ? (
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-4 text-xl sm:text-2xl font-medium text-gray-900 dark:text-white">Thank you for your inquiry!</h3>
                  <p className="mt-2 text-gray-600 dark:text-gray-300">We've received your message and will get back to you within 24 hours.</p>
                  <div className="mt-6">
                    <button type="button" onClick={() => setSubmitSuccess(false)} className="inline-flex items-center px-6 py-3 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-primary hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary/50">Send Another Message</button>
                  </div>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                  <div className="grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-2">
                    <div>
                      <label htmlFor="name" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 sm:mb-2">Full Name *</label>
                      <input type="text" name="name" id="name" value={formData.name} onChange={handleInputChange} placeholder="John Smith" className={`block w-full px-4 py-3 sm:py-2 rounded-lg text-base sm:text-sm ${errors.name ? 'border-red-300 text-red-900 placeholder-red-300' : 'border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-white'} shadow-sm focus:ring-primary focus:border-primary`} />
                    ...
                    <div>
                      ...
                      <select name="eventType" id="eventType" value={formData.eventType} onChange={handleInputChange} className={`mt-1 block w-full rounded-md ${formData.eventType ? 'border-primary dark:border-primary-600' : 'border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-white'} shadow-sm focus:ring-primary focus:border-primary`}>
                  ...
                  <button type="submit" disabled={isSubmitting} className={`group w-full inline-flex items-center justify-center px-6 py-3 text-base font-medium rounded-lg text-white bg-primary hover:bg-primary/90 active:bg-primary/95 hover:opacity-90 active:opacity-80 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2 transition-all duration-300 no-underline ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''}`}>
                    <span className="flex items-center justify-center">
                      <Send className="mr-2 w-5 h-5 transition-transform duration-300 ease-in-out will-change-transform transition-colors group-hover:-rotate-[18deg] group-hover:scale-110 group-hover:text-amber-300 text-white" style={{ willChange: 'transform' }} aria-hidden="true" />
                      <span className="no-underline" style={{ textDecoration: 'none' }}>{isSubmitting ? 'Sending...' : 'Send Inquiry'}</span>
                    </span>
                  </button>
                </form>
              )}
          ...
          <div className="order-2 lg:order-1 space-y-12">
            <div className="bg-gray-50 dark:bg-gray-800/70 py-8 sm:py-12 px-4 sm:px-8 rounded-3xl shadow-xl">
              <div className="text-center mb-8 sm:mb-10">
                <h2 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white sm:text-4xl">Trusted by Event Planners & Hosts</h2>
                <p className="mt-3 max-w-2xl mx-auto text-lg sm:text-xl text-gray-500 dark:text-gray-300">Don't just take our word for it. Here's what our clients have to say.</p>
              </div>
            ...
            <div className="grid gap-4 sm:gap-6 md:gap-8 sm:grid-cols-2">
              {testimonials.slice(0, 4).map((testimonial) => (
                <div key={testimonial.id} className="group bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 flex flex-col transition-all duration-300 hover:shadow-xl transform hover:-translate-y-1">
                  <blockquote className="flex-grow">
                    <p className="text-gray-600 dark:text-gray-300 text-sm sm:text-base italic leading-relaxed">"{testimonial.quote}"</p>
                  </blockquote>
                  <div className="mt-4 sm:mt-6 flex items-center">
                    <div className="flex-shrink-0">
                      <img className="h-8 w-8 sm:h-10 sm:w-10 rounded-full object-cover shadow-sm" src={testimonial.image} alt={testimonial.author} loading="lazy" />
                    </div>
                    ...
                    <div className="ml-auto flex-shrink-0 flex items-center">{renderStars(testimonial.rating)}</div>
                  </div>
                </div>
              ))}
            </div>
